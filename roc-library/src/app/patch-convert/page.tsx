...
export default function PatchConvertPage() {
  ...
  const captureCard = (key: string) => {
    console.log("Capture card:", key);
  };
  ...
  return (
    <div className="w-full grid grid-col-1 justify-center mt-20">
      {entries.map(([key, item]) => (
        <div key={key} className="w-full">
          <button
            onClick={() => captureCard(key)}
            className="bg-linear-to-r text-white from-pink-500 to-purple-500 px-4 py-2 rounded-md font-semibold hover:brightness-110"
          >
            💾 Download Image
          </button>
          ...
          <div
            id={`item${key}`}
            className="lg:max-w-5xl w-full mx-auto mt-6 p-6 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] bg-linear-to-tr from-[#fd6c01] to-[#f0d29c] backdrop-blur-md border border-orange-200 flex flex-col gap-8 transition-all duration-300 hover:scale-[1.02]"
          >
